...
<div class="card mt-3" style="width: 18rem;">
    <div class="card-body">
        ...
        <a href="<?= dirname($_SERVER['SCRIPT_NAME']); ?>/article/edit/<?= $article->getId(); ?>" class="card-link">Edit
            Article</a>
        <a href="<?= dirname($_SERVER['SCRIPT_NAME']); ?>/article/delete/<?= $article->getId(); ?>"
...
<div class="form">
    <form action="<?= dirname($_SERVER['SCRIPT_NAME']); ?>/comments/add/<?= $article->getId() ?>" method="post">
        <div class="mb-3">
            ...
            <input style="width:0;height:0;border:none;background-color:none;" type="text"
                name="articles_id" value="<?= $article->getId(); ?>">
        </div>
...
<p><a class="btn btn-success"
        href="<?= dirname($_SERVER['SCRIPT_NAME']); ?>/comments/<?= $article->getId() ?>">Редактировать комментарии</a>
</p>
